...
void modem_crash_msa_dump_rmb_regs(struct q6v5_data *drv,int ramdump_enable)
{
	if(!ramdump_enable) 
		return;
	...
	pr_info("%s+++\n", __func__);
	if(NULL == drv)
		pr_info("%s, struct q6v5_data *drv is NULL pointer!\n",__func__);
	else
		pil_msa_dump_rmb_regs(drv);
	pr_info("%s---\n", __func__);
}

EXPORT_SYMBOL(modem_crash_msa_dump_rmb_regs);
